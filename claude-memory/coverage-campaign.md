...
~/.pyenv/versions/3.13.5/bin/python -m poetry run pytest -q --import-mode=importlib \
  --cov=mobsf --cov-config=.coveragerc --cov-report=term-missing \
  mobsf/RBAC/tests mobsf/Analytics/tests mobsf/StaticAnalyzer/test_integration.py mobsf/**/test_cov_*.py
```
- Shell is **zsh**: an unquoted `$VAR` holding space-joined paths does NOT word-split — use the `mobsf/**/test_cov_*.py` glob, not a variable.
...
**Coverage reached: 25% → 66.9%** real-execution line coverage (strict, zero mocks). Per-pkg: StaticAnalyzer 82%, MobSF 83%, RBAC 86%, Analytics 93%, MalwareAnalyzer 69%, DynamicAnalyzer 27%, install 13%.
