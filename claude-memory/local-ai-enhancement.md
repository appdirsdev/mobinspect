...
**Runtime gotcha:** Homebrew `ollama` 0.30.6 on this Mac is BROKEN — `llama-server` runner binary missing, `ollama run` → 500. Ran the model via **`brew install llama.cpp` + `llama-cli -m <gguf-blob>`** instead (the ollama-pulled blob at `~/.ollama/models/blobs/sha256-6c0268…` IS a valid GGUF). For the real integration, use the official Ollama.app, not the brew formula. Deploy split: embeddings on prod VM; LLM generation on a 16GB on-prem box (8B needs ~5GB). Config via a `MOBINSPECT_AI_URL` env flag following existing `MOBINSPECT_/MOBINSPECT_` conventions. Related: [[release-2026-7]], [[server-deployment]].
