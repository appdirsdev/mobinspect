...
- [ ] Source CSS at `mobinspect/static/mobinspect/css/app.css` with `@layer base`, `@layer components`, `@layer utilities`
- [ ] Build script `scripts/tailwind-build.sh` (one-shot) and `scripts/tailwind-watch.sh` (dev)
- [ ] Vendor Alpine, HTMX, Motion One, Chart.js, Lucide into `mobinspect/static/mobinspect/vendor/` with `LICENSES.md`
- [ ] Add `mobinspect/MobInspect/templatetags/lucide.py` for inline-SVG icon rendering
- [ ] Add `mobinspect/MobInspect/templatetags/theme.py` for the no-flicker theme bootstrap script
...
- [ ] New Django app `mobinspect.RBAC` (initially placed alongside `mobinspect/`, renamed in Phase 3)
...
- [ ] Pass 1: Python package `mobinspect` → `mobinspect`
...
| GPL compliance slipups during rebrand | Lint rule: every source file must have an unchanged MobInspect copyright line |
