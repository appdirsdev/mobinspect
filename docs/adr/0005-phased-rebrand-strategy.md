...
- Pages are migrated in dependency order: foundation → reports → analytics → settings. If we rebranded first, every page touch repeats `s/MobSF/MobInspect/`
...
- Ship branches that are usable at every step — even before rebrand, Phase 2 + RBAC means we have a much-improved MobSF; if the rebrand were first, we'd have a renamed-but-otherwise-unchanged product, which is the worst combination
...
### Cons
- The branding inconsistency (we're calling it MobInspect in docs but the package is still `mobsf` for weeks) is awkward. Mitigated by clear status flags in templates (`{% comment %}TODO Phase 3 rebrand{% endcomment %}` is *not* used; we just leave the strings as-is and let Phase 3 sweep them).
- Anyone joining mid-stream has to know "we are MobInspect even though the code says MobSF". Documented in the README before Phase 0 lands.
