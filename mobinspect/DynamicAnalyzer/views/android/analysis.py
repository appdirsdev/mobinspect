...
def run_analysis(apk_dir, md5_hash, package):
    ...
    clip = Path(apk_dir) / 'mobinspect_app_clipboard.txt'
    if clip.exists():
...
def get_tls_logs(apk_dir, md5_hash):
    ...
    out = Path(apk_dir) / 'mobinspect_tls_tests.json'
    if not out.exists():
        return None
...
def get_log_data(apk_dir, package):
    ...
    apimon = os.path.join(apk_dir, 'mobinspect_api_monitor.txt')
    fd_logs = os.path.join(apk_dir, 'mobinspect_frida_out.txt')
