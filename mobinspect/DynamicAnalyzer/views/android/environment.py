...
from mobinspect.DynamicAnalyzer.views.common.frida.server_update import (
    FridaServerUpdater,
)
...
class Environment:
    ...
    def __init__(self, identifier=None):
        ...
        self.frida_str = (
            ...
            .encode('utf-8'))
        self.xposed_str = b'MobInspect-Xposed'
    ...
    def is_device_connected(self):
        """Return True only if the device reports the 'device' state.

        `adb get-state` prints 'device' when the target is online and ready.
        Any other state (offline, unauthorized, no device) or a non-zero
        exit means we cannot safely claim an operation succeeded.
        """
        if not self.identifier:
            return False
        try:
            out = subprocess.check_output(
                ...
                stderr=subprocess.STDOUT,
                timeout=10)
            return out.decode('utf-8', 'ignore').strip() == 'device'
        except Exception:
            logger.warning('Device %s is not in "device" state',
                           self.identifier)
            return False
    ...
    def configure_proxy(self, project, request):
        """HTTPS Proxy."""
        self.install_mobinspect_ca('install')
    ...
    def install_mobinspect_ca(self, action):
        """Install or Remove MobInspect Root CA."""
        mobinspect_ca = get_ca_file()
        ca_file = None
        if is_file_exists(mobinspect_ca):
            ca_construct = '{}.0'
            pem = open(mobinspect_ca, 'rb')
            ...
            return
        if action == 'install':
            logger.info('Installing MobInspect RootCA')
            ...
            logger.info('Removing MobInspect RootCA')
            self.adb_command(['rm',
                              ca_file], True)
    ...
    def screen_shot(self, outfile):
        ...
        Returns True only if the screenshot was captured on device and
        pulled to a non-empty local file. Returns False on any failure so
        callers do not report success when no artifact was produced.
        """
        if not self.is_device_connected():
            return False
        cap = self.adb_command(['screencap',
                                '-p',
        ...
        self.adb_command(['pull',
                          ...
                          outfile], False, True)
        # Verify the artifact was actually pulled to the host.
        if not os.path.exists(outfile) or os.path.getsize(outfile) == 0:
            logger.error('Screenshot was not saved to %s', outfile)
            return False
        return True
    ...
    def get_device_packages(self):
        ...
        for pkg_str in out.decode('utf-8').rstrip().split():
            ...
            if pkg == 'opensecurity.clipdump':
                # Do not include MobInspect agent
                continue
    ...
    def run_app(self, package):
        ...
        Returns True only if the device is online and monkey reported that
        it could launch the package. Returns False otherwise so callers do
        not report success when the app never started.
        """
        if not self.is_device_connected():
            return False
        out = self.adb_command(['monkey',
                                '-p',
                                package,
                                '-c',
                                ...
                                '1'], True)
        if out is None:
            logger.error('Failed to launch app %s', package)
            return False
        # monkey exits 0 even when it cannot launch; detect the abort message.
        if b'monkey aborted' in out or b'No activities found' in out:
            logger.error('App %s could not be launched: %s',
                         package, out.decode('utf-8', 'ignore').strip())
            return False
        return True
    ...
    def is_mobinspectyied(self, android_version):
        ...
        if android_version < 5:
            agent_file = '.mobinspect-x'
            agent_str = self.xposed_str
        else:
            agent_file = '.mobinspect-f'
    ...
    def mobinspecty_init(self):
        ...
        try:
            if version < 5:
                ...
                self.mobinspect_agents_setup('xposed')
            else:
                self.frida_setup()
                self.mobinspect_agents_setup('frida')
            logger.info('Preparation Completed!')
            return version
        except Exception:
            logger.exception('Failed to prepare Android instance')
            return False

    def mobinspect_agents_setup(self, agent):
        """Setup MobInspect agents."""
        create_ca()
        # Install MITM RootCA
        self.install_mobinspect_ca('install')
        if agent == 'frida':
    ...
    def xposed_setup(self, android_version):
        ...
        # Install MobInspect Agents for Xposed
        clip_dump_apk = os.path.join(self.tools_dir,
                                     xposed_dir,
                                     'ClipDump.apk')
        logger.info('Installing MobInspect Clipboard Dumper')
    ...
    def frida_setup(self):
        ...
        try:
            ...
            logger.error(
                'Cannot provision frida-server binary: %s. You will need'
                ' %s in %s for Dynamic Analysis to work',
                exc, frida_bin, settings.DWD_DIR)
            return
        if not stat:
