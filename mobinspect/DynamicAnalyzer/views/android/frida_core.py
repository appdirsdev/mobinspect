...
from mobinspect.DynamicAnalyzer.views.android.frida_scripts import (
    class_pattern,
    class_trace,
    get_loaded_classes,
    get_methods,
...
from mobinspect.DynamicAnalyzer.views.common.frida import (
    get_bridge_loader,
)
...
# Bounded retry for Frida spawn when the server is not yet running.
# Replaces an unbounded recursive self.spawn() loop that could blow the stack.
_SPAWN_MAX_ATTEMPTS = 3


class FridaServerStartError(RuntimeError):
    """Raised when the Frida server cannot be started after retries.

    View layer should catch this and surface a structured error message.
    """
...
class Frida:
    ...
    def __init__(self, app_hash, package, defaults, auxiliary, extras, code):
        ...
        self.api_mon = self.apk_dir / 'mobinspect_api_monitor.txt'
        self.frida_log = self.apk_dir / 'mobinspect_frida_out.txt'
        self.deps = self.apk_dir / 'mobinspect_app_deps.txt'
        self.clipboard = self.apk_dir / 'mobinspect_app_clipboard.txt'
    ...
    def frida_response(self, message, data):
        ...
        if 'payload' in message:
            msg = message['payload']
            api_mon = 'MobInspect-API-Monitor: '
            aux = '[AUXILIARY] '
            deps = '[RUNTIME-DEPS] '
            clip = 'mobinspect-android-clipboard:'
    ...
    def spawn(self):
        """Frida Spawn with bounded retry on ServerNotRunningError.

        Replaces an unbounded recursive retry that could cause stack
        overflow when the Frida server fails to start. Raises
        FridaServerStartError after _SPAWN_MAX_ATTEMPTS so callers
        (and the view layer) can render a structured error.
        """
        global _FPID
        env = Environment()
        last_err = None
        for attempt in range(1, _SPAWN_MAX_ATTEMPTS + 1):
            try:
                self.clean_up()
                ...
                logger.warning(
                    ...
                    attempt, _SPAWN_MAX_ATTEMPTS, backoff)
                if attempt < _SPAWN_MAX_ATTEMPTS:
                    time.sleep(backoff)
                continue
                ...
                return
        # Exhausted retries on ServerNotRunningError — surface a clear error.
        raise FridaServerStartError(
            'Frida server failed to start after '
            f'{_SPAWN_MAX_ATTEMPTS} attempts') from last_err
    ...
    def session(self, pid, package):
        ...
        try:
            try:
                ...
                try:
                    self.spawn()
                except FridaServerStartError:
                    logger.error(
                        'Frida server failed to start after %d attempts; '
                        'aborting session', _SPAWN_MAX_ATTEMPTS)
                    return
                session = device.attach(_FPID)
                time.sleep(2)
