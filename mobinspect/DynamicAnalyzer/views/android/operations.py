...
def mobinspecty(request, api=False):
    ...
    try:
        ...
        logger.exception('Preparing Android instance failed')
...
def execute_adb(request, api=False):
    """Execute ADB Commands (restricted to an allowlist)."""
    data = {'status': 'ok', 'message': ''}
    cmd = request.POST['cmd']
    if cmd:
        if not is_adb_command_allowed(cmd):
            logger.warning(
                'Denied adb subcommand not on allowlist: %r', cmd)
            denied = {
                'status': 'denied',
            ...
            if api:
                return denied
            # Return a 403 with a JSON body so the UI/clients can react.
            return HttpResponse(
                json.dumps(denied),
        ...
        try:
            proc = subprocess.Popen(
                args + cmd.split(),  # allowlisted above
...
def run_apk(request):
    ...
    try:
        ...
        if env.run_app(pkg):
            ...
            data = {
                'status': 'failed',
                'message': ('Failed to launch the app. Ensure the device '
...
def take_screenshot(request, api=False):
    ...
    try:
        ...
        outile = '{}screenshot-{}.png'.format(
            screen_dir,
            str(rand_int))
        if env.screen_shot(outile):
            logger.info('Screenshot captured')
            data = {'status': 'ok'}
        else:
            data = {
                'status': 'failed',
                'message': ('Failed to capture screenshot. Ensure the '
                            'device is connected.'),
            }
    except Exception as exp:
...
@permission_required(Permissions.SCAN)
@require_http_methods(['POST'])
def mobinspect_ca(request, api=False):
    """Install and Remove MobInspect Proxy RootCA."""
    data = {}
    try:
        env = Environment()
        action = request.POST['action']
        if action == 'install':
            env.install_mobinspect_ca(action)
        ...
        logger.exception('MobInspect RootCA Handler')
        data = {'status': 'failed', 'message': str(exp)}
    return send_response(data, api)
# AJAX
...
def global_proxy(request, api=False):
    ...
    try:
        ...
        logger.exception('MobInspect Global Proxy')
        data = {'status': 'failed', 'message': str(exp)}
    return send_response(data, api)
