...
import mobinspect.MalwareAnalyzer.views.Trackers as Trackers
from mobinspect.DynamicAnalyzer.views.android.analysis import (
    generate_download,
    get_screenshots,
    run_analysis,
)
from mobinspect.DynamicAnalyzer.views.android.dynamic_analyzer import (
    graceful_response,
)
from mobinspect.DynamicAnalyzer.views.android.operations import (
    get_package_name,
)
from mobinspect.DynamicAnalyzer.views.android.tests_xposed import (
    droidmon_api_analysis,
)
from mobinspect.DynamicAnalyzer.views.android.tests_frida import (
    apimon_analysis,
    dependency_analysis,
)
...
def view_report(request, checksum, api=False):
    ...
    try:
        ...
        if not is_file_exists(os.path.join(app_dir, 'logcat.txt')):
            # No dynamic-analysis artifacts on disk yet. This is an
            # expected "nothing to show" state, not an error — return a
            # friendly HTTP 200 page instead of a worker-recycling 500.
            msg = ('Dynamic Analysis report is not available '
                   'for this app. Perform Dynamic Analysis '
                   'and generate the report.')
            return graceful_response(
                request, msg, api, title='Dynamic Analysis Report')
        fd_log = os.path.join(app_dir, 'mobinspect_frida_out.txt')
