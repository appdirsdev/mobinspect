...
from mobinspect.DynamicAnalyzer.views.android.frida_core import (
    Frida,
...
def instrument(request, api=False):
    ...
    try:
        ...
        data['status'] = 'ok'
    except FridaServerStartError as exp:
        logger.error('Frida server start failed: %s', exp)
...
def live_api(request, api=False):
    try:
        ...
        if stream:
            ...
            apimon_file = os.path.join(apk_dir, 'mobinspect_api_monitor.txt')
            data = {}
...
def apimon_analysis(app_dir):
    ...
    try:
        strings = []
        location = os.path.join(app_dir, 'mobinspect_api_monitor.txt')
...
def dependency_analysis(package, app_dir):
    ...
    try:
        ...
        location = Path(app_dir) / 'mobinspect_app_deps.txt'
