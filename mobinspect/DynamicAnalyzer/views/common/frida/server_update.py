...
#   - default (MOBINSPECT_FRIDA_VERIFY=0): a LOUD warning is logged and the binary
#     is used anyway (backwards-compatible behaviour).
#   - fail-closed (MOBINSPECT_FRIDA_VERIFY=1): the binary is refused.
#
# ---------------------------------------------------------------------------
...
# MOBINSPECT_FRIDA_VERIFY=1 to enforce them.
# ---------------------------------------------------------------------------
FRIDA_SERVER_SHA256 = {
...
class FridaServerUpdater:
    ...
    def _verify_sha256(self, arch, file_path):
        ...
        Behaviour when no digest is pinned for the running version+arch:
          - settings.FRIDA_VERIFY is False (default, MOBINSPECT_FRIDA_VERIFY=0):
            log a LOUD warning and continue (backwards-compatible).
          - settings.FRIDA_VERIFY is True (MOBINSPECT_FRIDA_VERIFY=1): delete the
        ...
        if not expected:
            ...
            if fail_closed:
                ...
                raise RuntimeError(
                    ...
                    'MOBINSPECT_FRIDA_VERIFY=1 (fail-closed).')
            logger.warning(
                '%s Running it anyway because MOBINSPECT_FRIDA_VERIFY is not set; '
                'set MOBINSPECT_FRIDA_VERIFY=1 to fail closed.', msg)
            return
