...
from mobinspect.DynamicAnalyzer.views.ios.corellium_apis import (
    CorelliumAPI,
    CorelliumAgentAPI,
...
def list_apps(request, api=False):
    ...
    try:
        ...
        for i in r.get('apps'):
            ...
            dump_file = Path(
                settings.UPLD_DIR) / checksum / 'mobinspect_dump_file.txt'
...
def get_container_path(request, api=False):
    ...
    try:
        ...
        cfile = 'mobinspect_app_container_path.txt'
...
def ssh_execute(request, api=False):
    """Execute commands in VM over SSH (restricted to an allowlist)."""
    global SSH_TARGET
    res = ''
    data = {
        'status': 'failed',
