...
class IOSConnector:
    ...
    def _ssh_execute_command(self, command, timeout=30):
        ...
        result = {'output': '', 'error': '', 'exit_status':
                  - 1, 'exception': None, 'channel': None}
    ...
    def connect_usb(self, device_id, username, password, port=2222):
        ...
        try:
            ...
            if not self._check_usbmuxd():
                raise Exception(
                    'usbmuxd/iproxy not available. Install libimobiledevice.')
            ...
            self._establish_ssh_connection(
                'localhost', port, username, password, 'usb', device_info)
    ...
    def connect_wifi(self, ip_address, username, password, port=22):
        ...
        try:
            ...
            # Establish SSH connection
            self._establish_ssh_connection(
                ip_address, port, username, password, 'wifi', device_info)
            return True
        except Exception as e:
    ...
    def _establish_ssh_connection(
            self, host, port, username, password, connection_type, device_info):
    ...
    def _setup_usb_port_forward(self, device_id, port):
        ...
        try:
            ...
            if ssh_port_open and frida_port_open:
                logger.info(
                    'Port forwarding already exists: '
                    'localhost:%s -> device:22, localhost:37042 -> device:27042',
                    port,
                )
                return True
            ...
            if not ssh_port_open or not frida_port_open:
                ...
                raise Exception(
                    f"Port forwarding failed for: {', '.join(failed_ports)}")
