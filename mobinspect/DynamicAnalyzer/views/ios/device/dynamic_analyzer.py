...
from mobinspect.DynamicAnalyzer.forms import UploadFileForm
from mobinspect.DynamicAnalyzer.views.ios.helpers import (
    configure_proxy,
    get_local_ipa_list,
)
from mobinspect.DynamicAnalyzer.views.ios.device.device import IOSDevice
from mobinspect.DynamicAnalyzer.views.ios.device.connect import IOSConnector
from mobinspect.DynamicAnalyzer.views.ios.device.environment import IOSEnvironment
from mobinspect.DynamicAnalyzer.views.ios.device.frida import FridaIOSDevice
...
def dynamic_analysis_device(request, api=False):
    ...
    try:
        ...
        if env('MOBINSPECT_PLATFORM') == 'docker':
            devices = []
        else:
        ...
        context = {
            ...
            'title': 'MobInspect Dynamic Analysis Jailbroken Device',
...
@login_required
@permission_required(Permissions.SCAN)
@require_permission('dynamic.adb.shell')
@require_http_methods(['POST'])
def ssh_execute_device(request, api=False):
    """Execute commands in iOS device over SSH (restricted to an allowlist)."""
    data = {
        'status': FAILED,
    ...
    try:
        ...
        if not is_ssh_command_allowed(cmd):
            logger.warning(
                'Denied ssh subcommand not on allowlist: %r', cmd)
            denied = {
                'status': 'denied',
                'message': 'adb subcommand not on allowlist',
            }
            if api:
                return denied
            return HttpResponse(
...
def get_ios_devices_over_wifi():
    ...
    connect_strings_env = env(
        'MOBINSPECT_IOS_ANALYZER_IDENTIFIERS')
