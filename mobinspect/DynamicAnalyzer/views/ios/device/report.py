...
from mobinspect.DynamicAnalyzer.views.ios.analysis import (
    get_screenshots,
    ios_api_analysis,
    run_analysis,
)
from mobinspect.DynamicAnalyzer.views.ios.device.dynamic_analyzer import (
    FAILED,
    OK,
...
def ios_api_monitor(request, api=False):
    try:
        ...
        if stream:
            app_dir = Path(settings.UPLD_DIR) / checksum
            apimon_file = app_dir / 'mobinspect_dump_file.txt'
            data = {}
