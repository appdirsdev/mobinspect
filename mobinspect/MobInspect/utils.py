...
from mobinspect.MobInspect. init import api_key, env
...
# MobInspect Custom regex to catch maximum URI like strings
...
def print_version():
    """Print MobInspect Version."""
    logger.info(settings.BANNER)
    ver = settings.MOBINSPECT_VER
    # Never print the full legacy global REST API key to the log/journal —
    # it would leak a long-lived credential in plaintext on every startup.
    # Show only a short masked prefix so operators can sanity-check which
    # key is active without exposing it.
    ...
    if platform.system() == 'Windows':
        ...
        logger.info(
            '%sMobInspect %s%s', Color.GREY, ver, Color.END)
        print(f'REST API Key: {Color.BOLD}{masked_api_key}{Color.END}')
    os = platform.system()
...
def docker_translate_localhost(identifier):
    ...
    if not identifier:
        return identifier
    if not env('MOBINSPECT_PLATFORM') == 'docker':
        return identifier
    try:
...
def get_active_adb_connection_device():
    """Return the host:port of the active Android AdbConnection, or None.

    Looks up the dashboard-configured (RBAC.AdbConnection) active Android
    device, best-effort `adb connect`s to it so a remote emulator/device is
    attached before use, and returns its validated host_port. The model is
    imported lazily so this module stays importable pre-migrate and avoids
    app-loading/circular-import issues. Any failure returns None so the
    caller falls through to the existing precedence (settings/autodetect).
    """
    try:
        from django.apps import apps
        adb_connection = apps.get_model('rbac', 'AdbConnection')
        conn = adb_connection.objects.filter(
            is_active=True, platform='android').first()
        if not conn:
            return None
        # Best-effort connect so the remote device is attached before use.
        # Ignore failures here; selecting the host_port is still valid and
        # downstream adb usage will surface a real connectivity error.
        try:
            subprocess.run(
                [get_adb(), 'connect', conn.host_port],
                capture_output=True, timeout=10, check=False)
        except Exception:
            logger.warning(
                'Best-effort adb connect to active connection failed')
        return conn.host_port
    except Exception:
...
def get_device():
    ...
    active_conn = get_active_adb_connection_device()
    if active_conn:
        return docker_translate_localhost(active_conn)
...
def get_adb():
    ...
    try:
        ...
        if ADB_PATH:
            os.environ['MOBINSPECT_ADB'] = ADB_PATH
        else:
            os.environ['MOBINSPECT_ADB'] = 'adb'
...
def check_basic_env():
    """Check if we have basic env for MobInspect to run."""
    logger.info('MobInspect Basic Environment Check')
    try:
...
def get_android_dm_exception_msg():
    return (
        'Is your Android VM/emulator running? MobInspect cannot'
        ...
        ' MOBINSPECT_ANALYZER_IDENTIFIER')
