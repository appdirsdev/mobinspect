...
# of the X-MobInspect-Api-Key header without disrupting legitimate clients.
...
def _global_key_matches(presented):
    ...
    return compare_digest(mobinspect_api_key, presented)
...
def api_auth(request):
    ...
    try:
        from mobinspect.RBAC.models import ApiKey  # local import — avoid app load order
...
def _has_api_use_permission(request):
    ...
    if getattr(api_user, 'is_superuser', False) or getattr(
        ...
        return True
    try:
        from mobinspect.RBAC.permissions import has_permission
