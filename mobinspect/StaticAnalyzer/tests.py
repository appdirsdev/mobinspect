...
def api_test():
    ...
    auth = api_key(settings.MOBINSPECT_HOME)
    try:
        uploaded = []
        ...
        if resp.status_code == 200:
            ...
            return True
        resp = http_client.get('/api/v1/scans', HTTP_X_MOBINSPECT_API_KEY=auth)
        if resp.status_code == 200:
            ...
            return True
        resp = http_client.get(
            '/api/v1/scans?page=1&page_size=10', HTTP_X_MOBINSPECT_API_KEY=auth)
        if resp.status_code == 200:
        ...
        for pdf in pdfs:
            ...
            resp_custom = http_client.post(
                '/api/v1/download_pdf', pdf, HTTP_X_MOBINSPECT_API_KEY=auth)
            assert (resp.status_code == 200)
        ...
        for jsn in pdfs:
            ...
            resp_custom = http_client.post(
                '/api/v1/report_json', jsn, HTTP_X_MOBINSPECT_API_KEY=auth)
            assert (resp.status_code == 200)
            assert (resp_custom.status_code == 200)
            if (resp.status_code == 200
        ...
        for scr in pdfs:
            ...
            resp_custom = http_client.post(
                '/api/v1/scorecard', scr, HTTP_X_MOBINSPECT_API_KEY=auth)
        ...
        for sfile in files:
            ...
            resp_custom = http_client.post(
                '/api/v1/view_source', sfile, HTTP_X_MOBINSPECT_API_KEY=auth)
            assert (resp.status_code == 200)
            assert (resp_custom.status_code == 200)
            if resp.status_code == 200:
                dat = json.loads(resp.content.decode('utf-8'))
        ...
        resp_custom = http_client.post(
            ...
            HTTP_X_MOBINSPECT_API_KEY=auth)
        assert (resp_custom.status_code == 200)
        if resp.status_code == 200:
