...
mobinspect.StaticAnalyzer.views.android.converter.
...
class Dex2SmaliTests(TestCase):
    ...
    def test_disabled_returns_early(self):
        # settings_enabled() reads the real mobinspect.MobInspect.settings *module*
        ...
        tmp = tempfile.mkdtemp()
        _extract_classes_dex(tmp)
        original = mobinspect_settings.DEX2SMALI_ENABLED
        mobinspect_settings.DEX2SMALI_ENABLED = '0'
        try:
            self.assertIsNone(
                dex_2_smali(checksum, tmp + '/', TOOLS_DIR))
        finally:
            mobinspect_settings.DEX2SMALI_ENABLED = original
