...
def delete_suppression(request, api=False):
    ...
    try:
        ...
        rule = request.POST['rule']
        stype = request.POST['type']
        # `kind` is optional. When omitted, the rule is removed from both
        # rule-id and file suppressions so delete mirrors suppress, which
        # only requires {rule, type, hash}. When provided ('rule'/'file')
        # the removal is scoped to that store for backward compatibility.
