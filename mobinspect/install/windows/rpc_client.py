"""MobInspect rpc_client for static windows app analysis."""
import base64
...
def _init_key():
    ...
    pub_key = rsa.PublicKey.load_pkcs1(
        open(config['MobInspect']['pub_key']).read())
...
def binskim(sample, signature):
    ...
    command = 'analyze'
    path = config['MobInspect']['samples'] + sample
    output_p = '-o'
    output_d = config['MobInspect']['samples'] + sample + '_binskim'
    verbose = '--verbose'
    policy_p = '--config'
...
def binscope(sample, signature):
    ...
    target = [config['MobInspect']['samples'] + sample]
    out_type = ['/Red', '/v']
...
if __name__ == '__main__':
    ...
    config.read(expanduser('~') + '\\MobInspect\\Config\\config.txt')
