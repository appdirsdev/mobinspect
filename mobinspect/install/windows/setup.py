...
Setup script for the Windows vm for usage with MobInspect for
...
CONFIG_URL = (
    ...
    'mobinspect/main/'
    'mobinspect/install/windows/config.txt'
)
...
def create_folders():
    """Create MobInspect dirs."""
    print('[*] Creating other folders...')

    if not os.path.exists(CONFIG['MobInspect']['downloads']):
        os.makedirs(CONFIG['MobInspect']['downloads'])

    if not os.path.exists(CONFIG['MobInspect']['tools']):
        os.makedirs(CONFIG['MobInspect']['tools'])

    if not os.path.exists(CONFIG['MobInspect']['samples']):
        os.makedirs(CONFIG['MobInspect']['samples'])

    if not os.path.exists(CONFIG['MobInspect']['key_path']):
        os.makedirs(CONFIG['MobInspect']['key_path'])
...
def tools_nuget():
    ...
    mobinspect_subdir_tools = CONFIG['MobInspect']['tools']
    nuget_file_path = CONFIG['nuget']['file']

    # Open File
    nuget_file_local = open(os.path.join(
        mobinspect_subdir_tools, nuget_file_path), 'wb')
...
def tools_binskim():
    ...
    mobinspect_subdir_tools = CONFIG['MobInspect']['tools']
    nuget = mobinspect_subdir_tools + CONFIG['nuget']['file']
    ...
    # Search for the exes
    binaries = _find_exe(mobinspect_subdir_tools + folder, [])
    if len(binaries) != 2:
...
def tools_rpcclient():
    """Download and install rpc-server for MobInspect."""
    rpc_url = CONFIG['rpc']['url']
    mobinspect_subdir_tools = CONFIG['MobInspect']['tools']
    rpc_file_path = CONFIG['rpc']['file']

    # Open File
    rpc_local_file = open(mobinspect_subdir_tools + rpc_file_path, 'wb')
...
def tools_binscope():
    """Download and install Binscope for MobInspect."""
    mobinspect_subdir_tools = CONFIG['MobInspect']['tools']
    binscope_path = mobinspect_subdir_tools + 'BinScope'
...
def generate_secret():
    ...
    priv_key_file = open(CONFIG['MobInspect']['priv_key'], 'w')
    priv_key_file.write(privkey.save_pkcs1().decode('utf-8'))
    priv_key_file.close()
    pub_key_file = open(CONFIG['MobInspect']['pub_key'], 'w')
    ...
    print((
        ...
        '\t(default: mobinspect/MobInspect/windows_vm_priv_key.asc)'
        .format(CONFIG['MobInspect']['priv_key'], config_path)
    ))
...
def autostart():
    ...
    @echo off
    python "{}" %*
    pause""".format(mobinspect_subdir_tools + rpc_file)
...
def _place_lockfile(mobinspect_home):
    path = os.path.join(mobinspect_home, 'setup_done.txt')
    open(path, 'a').close()
...
def local_config():
    ...
    # Copy predefined config to MobInspect folder
    shutil.copy(
        os.getcwd() + '\\mobinspect\\install\\windows\\config.txt',
        os.path.join(CONFIG_PATH, CONFIG_FILE),
    )


def rewrite_local_config(mobinspect_home):
    """For local installation some config-vars need to be rewritten."""
    CONFIG['MobInspect']['subdir_tools'] = (
        mobinspect_home
        + '\\mobinspect\\StaticAnalyzer\\tools\\windows\\')
    CONFIG['MobInspect']['dir'] = mobinspect_home
...
def rewrite_config():
    ...
    CONFIG['MobInspect']['dir'] = expanduser('~') + CONFIG['MobInspect']['dir']

    # Rewrite config with new base path
    CONFIG['MobInspect']['downloads'] = CONFIG['MobInspect'][
        'dir'] + CONFIG['MobInspect']['downloads']
    CONFIG['MobInspect']['tools'] = CONFIG['MobInspect'][
        'dir'] + CONFIG['MobInspect']['tools']
    CONFIG['MobInspect']['samples'] = CONFIG['MobInspect'][
        'dir'] + CONFIG['MobInspect']['samples']
    CONFIG['MobInspect']['key_path'] = CONFIG['MobInspect'][
        'dir'] + CONFIG['MobInspect']['key_path']
    CONFIG['MobInspect']['priv_key'] = CONFIG['MobInspect'][
        'key_path'] + CONFIG['MobInspect']['priv_key']
    CONFIG['MobInspect']['pub_key'] = CONFIG['MobInspect'][
        'key_path'] + CONFIG['MobInspect']['pub_key']
...
def install_locally(mobinspect_home):
    """Install the MobInspect-Utils on the same system as MobInspect."""
    local_config()
    read_config()
    rewrite_config()
    create_folders()
    ...
    tools_binskim()
    tools_binscope()
    _place_lockfile(mobinspect_home)


def _install_remote():
    """Install the MobInspect-Utils on a Windows-VM for static analysis."""
    download_config()
    read_config()
    rewrite_config()
    create_folders()
