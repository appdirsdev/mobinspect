"""
MobInspect — inline Lucide icon template tag.

Usage in templates:
    {% load mi_icons %}
    {% icon "shield" %}
    {% icon "shield" size=20 class="text-mobinspect-500" %}

Icons render as inline SVG so they inherit `currentColor` from CSS.
The icon catalog is intentionally curated — adding a new icon means
adding its path data to ICON_PATHS below. This keeps the template
tag self-contained (no filesystem reads, no caching to manage).

Path data is taken verbatim from https://lucide.dev (ISC license,
attribution preserved in vendor/LICENSES.md).
"""
...
# Inner SVG markup for each icon (no <svg> wrapper — that's added below).
# Stroke is currentColor; the wrapper sets stroke-width.
ICON_PATHS = {
    # Navigation
    'gauge': '<path d="M12 14 4 22"/><path d="M3.34 19a10 10 0 1 1 17.32 0"/><circle cx="12" cy="14" r="2"/>',
    ...
    'git-compare': '<circle cx="5" cy="6" r="3"/><circle cx="19" cy="18" r="3"/><path d="M13 6h3a2 2 0 0 1 2 2v7"/><path d="M11 18H8a2 2 0 0 1-2-2V9"/>',
...
def icon(name, size=20, **kwargs):
    ...
    Unknown icon names render an empty 'help-circle' so the layout
    doesn't break in development.
    """
    paths = ICON_PATHS.get(name) or ICON_PATHS['help-circle']
    extra_class = kwargs.get('class', '')
    stroke_width = kwargs.get('stroke', '1.5')
    aria_label = kwargs.get('label', '')
    aria = (
        f' role="img" aria-label="{aria_label}"' if aria_label
    ...
    return mark_safe(
        ...
        f'class="inline-block flex-shrink-0 {extra_class}"{aria}>'
        f'{paths}'
        f'</svg>',
    )
