"""Real-execution coverage tests for mobsf.MobSF.init.
...
from mobsf.MobSF import init as mod
...
# Absolute path to the real repo base dir (mobsf/ package dir).
BASE_DIR = Path(__file__).resolve().parent.parent  # .../MobInspect/mobsf

# Env var names touched by the module; snapshot/restore around each test.
_ENV_KEYS = [
    'MOBINSPECT_SECRET_KEY', 'MOBSF_SECRET_KEY',
    'MOBINSPECT_API_KEY', 'MOBSF_API_KEY',
    'MOBINSPECT_API_KEY_FILE', 'MOBSF_API_KEY_FILE',
...
def test_env_default_when_unset():
    assert env('MI_ENV_TEST_NEW', 'MI_ENV_TEST_OLD', default='def') == 'def'
    assert env('MI_ENV_TEST_NEW') is None


def test_env_legacy_name_emits_deprecation_and_returns_value():
    # Reset one-time warning state so the warning path executes.
    if hasattr(env, '_warned'):
        env._warned.discard('MI_ENV_TEST_OLD')
    os.environ['MI_ENV_TEST_OLD'] = 'legacy'
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter('always')
        val = env('MI_ENV_TEST_NEW', 'MI_ENV_TEST_OLD')
    assert val == 'legacy'
    assert any(issubclass(w.category, DeprecationWarning) for w in caught)
    # Second call: value still returned, warning suppressed (already warned).
    assert env('MI_ENV_TEST_NEW', 'MI_ENV_TEST_OLD') == 'legacy'
...
def test_get_mobsf_version():
    banner, version, vversion = get_mobsf_version()
...
def test_get_secret_from_file_or_env_direct_env():
    os.environ['MOBSF_API_KEY'] = 'plainvalue'
    assert get_secret_from_file_or_env('MOBSF_API_KEY') == 'plainvalue'


def test_get_secret_from_file_or_env_file_variant(tmp_path):
    secret = tmp_path / 's'
    secret.write_text('filevalue\n')
    os.environ['MOBSF_API_KEY_FILE'] = secret.as_posix()
    assert get_secret_from_file_or_env('MOBSF_API_KEY') == 'filevalue'
...
def test_api_key_from_legacy_env_variable(tmp_path):
    os.environ['MOBSF_API_KEY'] = 'legacyapikey'
...
def test_get_mobsf_home_base_dir_mode_creates_subdirs(tmp_path):
    home = get_mobsf_home(False, tmp_path.as_posix())
...
def test_get_mobsf_home_use_home_with_custom_home_dir(tmp_path):
    home_dir = tmp_path / 'customhome'
    home_dir.mkdir()
    os.environ['MOBINSPECT_HOME_DIR'] = home_dir.as_posix()
    # Use a base_dir lacking signatures + MobSF/settings.py so copytree and
    # create_user_conf hit their swallowed-exception paths (no crash).
    base = tmp_path / 'base'
    base.mkdir()
    home = get_mobsf_home(True, base.as_posix())
    assert home == home_dir.as_posix()
    for sub in ('downloads', 'screen', 'uploads', 'tools', 'signatures'):
        assert (home_dir / sub).is_dir()


def test_get_mobsf_home_exception_path_returns_none(tmp_path):
    ...
    afile.write_text('x')
    assert get_mobsf_home(False, afile.as_posix()) is None
