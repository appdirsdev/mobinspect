"""Real-execution (no-mock) coverage tests for mobsf.MobSF.tools_download.
...
from mobsf.MobSF import tools_download as td
from mobsf.MobSF.tools_download import (
...
class _Handler(BaseHTTPRequestHandler):
    body = b'MobSF-real-download-payload'
...
@contextmanager
def http_server(body=b'MobSF-real-download-payload', content_length=None):
...
def test_proxy_disabled_returns_empty_and_verifies():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED=None,
             MOBSF_UPSTREAM_PROXY_SSL_VERIFY=None):
...
def test_proxy_disabled_ssl_verify_off():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED='', MOBSF_UPSTREAM_PROXY_SSL_VERIFY='0'):
        proxies, verify = standalone_upstream_proxy()
    assert proxies == {}
...
def test_proxy_enabled_no_username():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED='1',
             MOBSF_UPSTREAM_PROXY_USERNAME=None,
             MOBSF_UPSTREAM_PROXY_PASSWORD=None,
             MOBSF_UPSTREAM_PROXY_TYPE='http',
             MOBSF_UPSTREAM_PROXY_IP='10.0.0.5',
...
def test_proxy_enabled_with_credentials():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED='1',
             MOBSF_UPSTREAM_PROXY_USERNAME='alice',
             MOBSF_UPSTREAM_PROXY_PASSWORD='s3cret',
             MOBSF_UPSTREAM_PROXY_TYPE='https',
             MOBSF_UPSTREAM_PROXY_IP='192.168.1.9',
             MOBSF_UPSTREAM_PROXY_PORT='3128',
             MOBSF_PLATFORM=None):
        proxies, verify = standalone_upstream_proxy()
...
def test_proxy_enabled_docker_localhost_translation():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED='1',
             ...
             MOBSF_UPSTREAM_PROXY_PASSWORD=None,
             MOBSF_UPSTREAM_PROXY_TYPE='http',
             MOBSF_UPSTREAM_PROXY_IP='127.0.0.1',
             MOBSF_UPSTREAM_PROXY_PORT='3128',
...
def test_proxy_enabled_docker_localhost_name_translation():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED='1',
             MOBSF_UPSTREAM_PROXY_USERNAME=None,
             MOBSF_UPSTREAM_PROXY_IP='localhost',
             MOBSF_UPSTREAM_PROXY_PORT='3128',
             MOBSF_PLATFORM='docker'):
        proxies, _ = standalone_upstream_proxy()
...
def test_proxy_ssl_verify_quoted_one():
    with env(MOBSF_UPSTREAM_PROXY_ENABLED='', MOBSF_UPSTREAM_PROXY_SSL_VERIFY='"1"'):
...
def test_download_file_via_system_proxy_env(tmp_path):
    ...
    with http_server(body=payload) as url:
        ...
        with env(http_proxy=f'http://{host}:{port}',
                 ...
                 MOBSF_UPSTREAM_PROXY_ENABLED=None):
            downloaded = download_file('http://mobsf.example/file', str(dest))
    assert downloaded == len(payload)
    assert dest.read_bytes() == payload


def test_download_file_via_mobsf_upstream_proxy_ssl_off(tmp_path):
    # No system proxies -> MobSF upstream proxy branch. SSL_VERIFY=0 also
    ...
    with http_server(body=payload) as url:
        ...
        with env(http_proxy=None, https_proxy=None,
                 ...
                 MOBSF_UPSTREAM_PROXY_ENABLED='1',
                 MOBSF_UPSTREAM_PROXY_USERNAME=None,
                 MOBSF_UPSTREAM_PROXY_PASSWORD=None,
                 MOBSF_UPSTREAM_PROXY_TYPE='http',
                 MOBSF_UPSTREAM_PROXY_IP=host,
                 MOBSF_UPSTREAM_PROXY_PORT=port,
                 MOBSF_PLATFORM=None,
                 MOBSF_UPSTREAM_PROXY_SSL_VERIFY='0'):
            downloaded = download_file('http://mobsf.example/file', str(dest))
    assert downloaded == len(payload)
    assert dest.read_bytes() == payload
