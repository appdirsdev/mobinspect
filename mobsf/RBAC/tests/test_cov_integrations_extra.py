"""
Additional coverage tests for mobsf/RBAC/views.py and mobsf/RBAC/models.py.

Targets the specific branches left uncovered by test_cov_views.py,
test_cov_integrations.py and the rest of mobsf/RBAC/tests/:
...
mirrors mobsf/RBAC/tests/test_cov_integrations.py and
mobsf/RBAC/test_cov_views.py: a locally-defined superuser force_login'd
...
def test_validate_host_port_port_int_conversion_error(monkeypatch):
    ...
    monkeypatch.setattr(
        'mobsf.MobSF.utils.SSH_DEVICE_ID_REGEX', _FakeRegex(fake_match))
...
def test_run_adb_binary_not_found(monkeypatch):
    monkeypatch.setattr('mobsf.MobSF.utils.get_adb', lambda: None)
...
def test_run_adb_timeout(monkeypatch):
    monkeypatch.setattr('mobsf.MobSF.utils.get_adb', lambda: '/usr/bin/adb')
...
# in mobsf/RBAC/urls.py, and Django's <slug:...> converter matches pure-digit
