# -*- coding: utf_8 -*-
"""Coverage tests for mobsf/StaticAnalyzer/views/common/llm/signals.py.
...
from mobsf.StaticAnalyzer.views.common.llm import signals
from mobsf.StaticAnalyzer.views.common.llm.signals import (
    _SCAN_TASK_FUNCS,
    _func_name,
...
PATCH_TARGET = 'mobsf.StaticAnalyzer.views.common.llm.tasks.enrich_in_background'
...
def test_func_as_dotted_path_string_resolves_to_basename():
    ...
    dotted = 'mobsf.StaticAnalyzer.tasks.apk_analysis_task'
    with mock.patch(PATCH_TARGET) as enrich:
...
@pytest.mark.parametrize('func_name', [
    'static_analyzer',           # a real, non-scan-completion task in MobSF
    'unknown_task',
    '',
...
def test_import_failure_of_tasks_module_is_swallowed(monkeypatch):
    ...
    def _boom_import(name, *args, **kwargs):
        if name == 'mobsf.StaticAnalyzer.views.common.llm.tasks':
