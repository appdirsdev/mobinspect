...
- **Missing web fonts (cosmetic 404s).** `mobinspect/static/mobinspect/fonts/` was empty but `css/dist/app.css` `@font-face` references `Inter-Variable.woff2` and `JetBrainsMono-Variable.woff2`. Downloaded them from fontsource jsDelivr CDN into that dir. `STATIC_ROOT` == `mobinspect/static` (BASE_DIR is the mobinspect package), and whitenoise indexes static files once at startup (autorefresh pinned off), so a server restart is needed to serve newly added static files.
