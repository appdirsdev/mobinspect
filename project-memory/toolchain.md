...
MobInspect (a MobSF fork, Django app) local dev toolchain on this macOS (Apple Silicon) machine:
...
- **PostgreSQL 16** (`brew services`), role `mobsf` / db `mobsf` / password `mobsf`. App uses SQLite by default and only switches to Postgres when `POSTGRES_USER/PASSWORD/HOST` env vars are set (see `.env.postgres`).
