...
if [[ "$py_major" -ne 3 || "$py_minor" -lt 12 || "$py_minor" -gt 13 ]]; then
    echo "[ERROR] MobInspect dependencies require Python 3.12 - 3.13. You have Python ${python_version}."
    exit 1
fi
...
# Bootstrap the initial admin. Idempotent — re-runs do nothing if a
# superuser already exists. Set MOBINSPECT_ADMIN_PASSWORD ahead of time
# for non-interactive installs; otherwise this prompts on a TTY or
# generates a 24-char password and writes it to
# ~/.MobInspect/initial-admin-password.txt (mode 0600).
python3 -m poetry run python manage.py bootstrap_admin
