...
def test_upload_redirects_to_recent_scans(admin_page):
    ...
    Deliberately does NOT reuse the shared android.so fixture (see
    ``fixtures.data.SCANNED['so']``) — other specs assume that hash stays
    scanned and untouched, so this test uploads a private, uniquely-content
    scratch copy (android.so + a random suffix) that only it ever creates
    or deletes, keeping the two fully decoupled."""
    import uuid
    ...
    scratch_name = f'upload_scratch_{uuid.uuid4().hex[:8]}.so'
    ...
    try:
        with page.expect_response(
                ...
                timeout=30000):
            page.set_input_files('#uploadFile', scratch_path)

        page.wait_for_url(re.compile(r'/recent_scans/'), timeout=30000)
        expect(page).to_have_url(re.compile(r'/recent_scans/'))
        assert 'Traceback (most recent call last)' not in page.content()

        # Clean up: find the just-created row (top of Recent scans, matches
        # by file name) and delete it via the real endpoint, real CSRF token.
        row = page.locator('table tbody tr', has_text=scratch_name).first
        _delete_row_scan(page, row)
    finally:
        os.remove(scratch_path)


def _delete_row_scan(page, row):
    if row.count():
